function locomotiveAnimation() {
  ...
  function cursorAnimation() {
    ...
    document.addEventListener("mousemove", function(dets) {
      gsap.to(crsr, { x: dets.x, y: dets.y });
    });
    ...
    allImages.forEach(function(elem) {
      elem.addEventListener("mouseenter", function() {
        ...
        gsap.to(crsr, { width: "170px" });
        gsap.from("#cursor h5", { opacity: 0, delay: 0.2 });
        crsr.innerHTML = `<h5>${text}</h5> <h5>${text}</h5> <h5>${text}</h5>`;
      });
  
      elem.addEventListener("mouseleave", function() {
        crsr.innerHTML = "<h5></h5>";
        gsap.to(crsr, { width: "25px" });
      });
    });
  }
  
  // cursorAnimation() // Call the function here
  ...
  function page1Text() {
    ...
    gsap.from(".page1-text h1 span", { y: 300, opacity: 0, delay: 0.5, duration: 0.6, stagger: 0.1 });
  }
  ...
  elementContainer.addEventListener("mouseenter", function() {
    gsap.to("#moving-image", { opacity: 1 });
  });
  
  elementContainer.addEventListener("mouseleave", function() {
    gsap.to("#moving-image", { opacity: 0 });
  });
  ...
  allElements.forEach(function(elem) {
    elem.addEventListener("mouseenter", function() {
      ...
      gsap.to(moveImg, { attr: { src: image } });
    });
